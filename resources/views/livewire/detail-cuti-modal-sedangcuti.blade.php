@if($showModalSedangCuti)
<div class="modal fade show" style="display: block; background: rgba(0,0,0,0.5)" tabindex="-1">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Detail Sedang Cuti Hari Ini</h5>
                <button wire:click="closeModalSedangCuti" class="close">&times;</button>
            </div>
            <div class="modal-body">
                @if(count($jumlahSedangCuti) > 0)
                <div class="table-responsive">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @foreach($jumlahSedangCuti as $index => $cuti)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                @if ($cuti->karyawan)
                ...
                <div class="alert alert-info">
                    Tidak ada data karyawan sedang cuti saat ini.
                </div>
                @endif
            </div>
            <div class="modal-footer">
                <button wire:click="closeModalSedangCuti" class="btn btn-secondary">
                    Tutup
                </button>
            </div>
        </div>
